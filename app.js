...
const BT_STRAT_DESC = {
  'adaptive': {
    ...
    rules: [
      ...
      '<b>하락국면</b>(200일선 아래 + 200일선 하락) → <b>볼린저 보수형</b>: 하단 매수→중심선(20MA) 도달 시 빠르게 익절(반등만 짧게)',
    ],
    tip: '한 가지 전략은 특정 장세에서만 강합니다. 적응형은 국면을 읽어 전략을 바꿉니다. (개선: 하락장도 현금 대기 대신 볼린저 저점매수로 대응 — 하락장 반등 수익을 일부 챙기되 낙폭은 다소 늘 수 있음)',
  },
  '2425g': {
    t: '2425g (제미나이 개선안)',
    how: '2425를 제미나이 제안대로 다듬은 변형입니다. <b>고점 추격을 줄이고, 강한 상승은 끝까지 타는</b> 데 초점.',
    rules: [
      '<b>진입</b>: 2425 조건(200일선 위+정배열) + <b>RSI≤75일 때만</b> 매수(과열 추격 방지=눌림목)',
      '<b>청산</b>: 20&lt;60선 이탈 or 200일선 이탈 <b>또는 RSI가 80을 넘었다가 70 아래로 꺾일 때</b>',
      '핵심 차이=원본은 RSI 80에서 즉시 매도하지만, 이건 <b>과열이 꺾일 때까지 보유</b> → 급등주 대상승을 더 챙김',
    ],
  ...
  '2425b': {
    t: '2425b (매수=2425, 매도=볼린저 상단)',
    how: '매수 시점은 2425와 동일하되, <b>매도만 "볼린저 밴드 상단 터치"로 차익실현</b>하는 실험 변형. 200일선 이탈 시엔 안전 청산.',
    rules: [
      '<b>매수</b>: 2425와 동일(200일선 위 + 20&gt;60 정배열 + RSI≤80)',
      '<b>매도</b>: 주가가 <b>볼린저 상단(20MA+2σ)</b> 터치 시 익절 / 또는 200일선 이탈 시 청산',
      '추세를 끝까지 타는 대신 <b>고점(상단)에서 팔고 눌림에서 다시 사는 스윙형</b>으로 변함',
    ],
    tip: '실측: 완만한 추세주(AAPL)는 너무 일찍 팔아 손해(76%→29%, 매매 2.5배↑). 반대로 변동성 크거나 오르내림 잦은 종목(엔비디아·카카오·PayPal)은 스파이크 익절이 유효. 즉 "출렁이는 종목"에 적합. 매매가 2배로 늘어 수수료·세금 부담 주의.',
  },
  '2425': {
    t: '⭐ 2425 전략 (혼합 · 추천)',
